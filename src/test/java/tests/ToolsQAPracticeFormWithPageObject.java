...
public class ToolsQAPracticeFormWithPageObject {
    ...
    void selenideRegistrationForm() {
        ...
        $(byClassName("table-responsive")).shouldHave(text("Subjects"), text("Math"));
        $(byClassName("table-responsive")).shouldHave(text("Hobbies"), text("Sports" + ", " + "Music"));
        $(byClassName("table-responsive")).shouldHave(text("Picture"), text("1.jpg"));
        $(byClassName("table-responsive")).shouldHave(text("Address"), text("MyMainCurrentAddress"));
        $(byClassName("table-responsive")).shouldHave(text("State and City"), text("Haryana" + " " + "Karnal"));
        // ---зона проверок заполнения значений
